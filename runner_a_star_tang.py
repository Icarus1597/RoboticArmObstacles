...
# Calculate alpha for starting posture generation
...
def update(frame):
    ...
    if(distance_femur < config.min_distance_to_obstacle or distance_coxa < distance_to_obstacle):
        ...
        # Calculate Femur link Velocities for U_rep
    ...
    # Follow path generated by A* node by node
    if(path_node_list != -1):
    ...
    # If reached current node of path, move to next node
