...
def update(frame):
    """ Updates the frame. Calculates the new thetas for the Robotic Arm and updates its position based on potential field method
        while considering all links of the arm
    Args:
    ...
    if(current_time - start_time > config.timeout) :
        ...
        config.pf_all_links_number_timeout += 1
    ...
    point.set_data([config.target_x], [config.target_y])  # Update the position of the additional point
    obstacle_circle = plt.Circle(config.center, config.radius, fc='y')
    plt.figure(fig.number)
    ...
    if (distance_to_target) < config.delta_success_distance :
        ...
        config.pf_all_links_number_success += 1
        config.pf_all_links_list_covered_distance.append(covered_distance)
        config.pf_all_links_time_needed.append(time.time() - start_time)
        ani.event_source.stop()
        plt.figure(fig.number)
        plt.close()
        plt.figure(figure_distance_to_target.number)
